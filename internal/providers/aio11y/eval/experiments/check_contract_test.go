...
// cmd/gcx/root/aio11y_experiments_check_test.go. These rows cover only what
...
func TestCheck_OutputContract(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, "gcx.aio11y.experiment_check", doc["type"], "consumers dispatch on type")
...
func TestCheck_DefaultFormat(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			agentMode: true,
			want:      []string{"gcx.aio11y.experiment_check", "min_pass_rate"},
