...
func TestExperimentsCheck_EndToEnd(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, "gcx.aio11y.experiment_check", doc["type"])
...
// TestExperimentsCheck_ReportFieldFallback proves the command reads the legacy
// "run" key when a server omits "experiment".
func TestExperimentsCheck_ReportFieldFallback(t *testing.T) {
	srv, _ := newReportServer(t, 0, `{"run":{"run_id":"r-legacy","status":"completed"},"summary":{"test_case_count":100,"completed_count":100,"pass_rate":0.99,"pass_count":99,"pass_denominator":100}}`)
	stdout, _, err := runExperimentsCheck(t, srv.URL, "r-legacy", "--min-pass-rate", "0.9", "-o", "json")

	require.NoError(t, err)
	assert.Contains(t, stdout, `"experiment_id": "r-legacy"`)
	assert.Contains(t, stdout, `"verdict": "pass"`)
}
